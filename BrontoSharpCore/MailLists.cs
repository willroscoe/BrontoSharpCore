...
namespace Bronto.API
{
    public class MailLists : BrontoApiClass
    {
        ...
        public async Task<BrontoResult> AddToListAsync(mailListObject mailList, contactObject contact)
        {
            return await AddToListAsync(mailList, new contactObject[] { contact });
        }
        ...
        public BrontoResult Add(IEnumerable<mailListObject> mailLists)
        {
            ...
            {
                writeResult response = client.addListsAsync(session.SessionHeader, mailLists.ToArray());
